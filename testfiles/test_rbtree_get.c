...
void rbtree_testcase_1w(int connfd,int count) {
   // int count = 10000;
    printf("rbtree test\n");
    ...
    printf("rbtree_set %d--> time_used: %d ms, qps: %d\n", count,time_used, count * 1000 / time_used);
}
void rbtree_get_testcase_1w(int connfd,int count) {
    //int count = 10000;
    printf("rbtree test\n");
    struct timeval tv_begin, tv_end;
    gettimeofday(&tv_begin, NULL);
    ...
    for (int i = 0; i < count; i++) {
        ...
        sprintf(val, "V%d\r\n", i);
        char *cmd1[] = {"RGET", key};
        testcase(connfd, cmd1, 2, val, "RGET");
    }

    gettimeofday(&tv_end, NULL);
    int time_used = TIME_SUB_MS(tv_end, tv_begin);
    printf("rbtree_get %d--> time_used: %d ms, qps: %d\n", count,time_used, count * 1000 / time_used);
}
//test_rbtree_get 192.168.10.15 2000
int main(int argc, char *argv[]) {
    ...
    rbtree_get_testcase_1w(connfd,count);
