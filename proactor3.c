...
typedef struct connection_s{
    ...
    size_t woffset;              /* already-sent offset inside wbuffer */
    int write_pending;           /* whether a send SQE is outstanding */
    int wlength;
    int recv_pending;
} connection_t;
...
int queue_send(struct io_uring *ring,int fd,char *data,size_t len){
    ...
    if(c->wlength==0&&!c->write_pending){
        if(len>BUFFER_LENGTH){
            size_t tocopy=BUFFER_LENGTH;
            memcpy(c->wbuffer,data,tocopy);
            c->wlength=tocopy;
            c->woffset=0;
            c->write_pending=1;
            if(set_event_send(ring,fd,c->wbuffer,c->wlength,0)<0){
                fprintf(stderr,"[queue_send] set_event_send failed for fd %d\n",fd);
                c->write_pending=0;
                c->wlength=0;
                return -1;
            }
            printf("[queue_send] fd %d data too large, only sent %zu bytes\n",fd,tocopy);
            return -2;
        }else{
    ...
    if(c->wlength+len<=BUFFER_LENGTH){
        memcpy(c->wbuffer+c->wlength,data,len);
        c->wlength+=len;
        if(!c->write_pending){
            c->write_pending=1;
            if(set_event_send(ring,fd,c->wbuffer,c->wlength,0)<0){
        ...
        //     }
        // }
        /* 没有足够空间，返回错误（调用方可以重试或分段） */
        fprintf(stderr, "[queue_send] fd %d send buffer full (%d + %ld > %d)\n",
                fd, c->wlength, len, BUFFER_LENGTH);
        return -2;
    }
...
int proactor_broadcast( char *msg, size_t len) {
    ...
    for (int i = 0; i < client_count; i++) {
        ...
        if(rc==0||rc==-2){
            ...
            fprintf(stderr, "[proactor_broadcast] failed to send to fd %d\n", fd);
        }
    }
    if(any){
...
int proactor_start(unsigned short port, msg_handler handler) {
  ...
  //
            ...
            if(written>conn3->wlength-conn3->woffset){
                written=conn3->wlength-conn3->woffset;
            }
            conn3->woffset+=written;

            if(conn3->woffset>=conn3->wlength){
                // 全部发送完毕
                conn3->write_pending=0;
                conn3->wlength=0;
                conn3->woffset=0;
                ...
                size_t rem=conn3->wlength-conn3->woffset;
                if(set_event_send(&ring,result.fd,conn3->wbuffer+conn3->woffset,rem,0)<0){
                    fprintf(stderr, "[proactor_start] set_event_send failed for fd %d after partial write\n", result.fd);
                    close(result.fd);
                    conn3->write_pending=0;
                    continue;
                }
