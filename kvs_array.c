...
int kvs_array_set(kvs_array_t *inst, char *key, char *value) {
	...
	if (inst == NULL || key == NULL || value == NULL){
		printf("inst %d,key %s,value %s\n",inst->total,key,value);
		return -1;
	} 
...
void kvs_array_print(){
	int i=0;
	for(i=0;i<global_array.total;i++){
		printf("idx:%d %s:%s\n",i,global_array.table->key,global_array.table->value);
	}
}
