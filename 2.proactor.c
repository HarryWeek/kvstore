...
extern int kvs_ms_protocol(char *msg, int len, char *response);
    char pro_response[BUFFER_LENGTH];
	char recve[BUFFER_LENGTH];
#if ENABLE_MS
...
int proactor_connect(char *master_ip, unsigned short conn_port) {
	...
	int tmp=0;
				...
				int* rlen=get_conn_rlen(sockfd);
                set_event_recv(&ring, sockfd, buf+*rlen, BUFFER_LENGTH-*rlen, 0);
    ...
    while (1) {
        ...
        for (int i = 0; i < nready; i++) {
            ...
            if (result.event == EVENT_ACCEPT) {
				...
				int* rlen=get_conn_rlen(connfd);
                set_event_recv(&ring, connfd, buf+*rlen, BUFFER_LENGTH-*rlen, 0);
            ...
            // ==========================================
				...
				printf("rlen: %d ret: %d all: %d\n",*rlen,ret,*rlen+ret);
                // if (*rlen + ret > BUFFER_LENGTH) {
                //     // 缓冲区满了（极少），清空避免崩溃
                //     *rlen = 0;
                // }
				//printf("get: %s\n",recve);                
                //memcpy(buf + *rlen, recve, ret);
                *rlen += ret;
                //buf[*rlen] = '\0'; 
                // 方便调试（不破坏 buffer）
                ...
                //printf("get msg:%s rlen: %d\n",buf,*rlen);
					...
					// 	// 			BUFFER_LENGTH - *rlen,
					// 	// 			0);	
					// 	set_event_send(&ring, result.fd, NULL, 0, 0);					
					// 	continue;
					// }
                    ret = kvs_handler(packet, strlen(packet), pro_response);
					//printf("response ret:%d\n",ret);
					kvs_free(packet);
                    set_event_send(&ring, result.fd, pro_response, ret, 0);
            ...
            // ==========================================
                ...
                //printf("fd:%d\n",result.fd);
                // 写完不清空 buffer，只继续读
				printf("remain buffer:%s %d\n",conn->buffer,conn->rlength);
				//printf("remain length:%d %d\n",conn->rlength,*rlen);
                set_event_recv(&ring, result.fd,
                            conn->buffer + conn->rlength,
                            BUFFER_LENGTH - conn->rlength,
                            0);
				// memset(recve,0,BUFFER_LENGTH);
				// set_event_recv(&ring,result.fd,recve,BUFFER_LENGTH,0);
